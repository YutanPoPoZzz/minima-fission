...
- シーン = **原子**: 
  - 中央 = **原子核**（発光オーブ7個前後のクラスタ、緑と黄が混ざる、呼吸アニメ）= 再生トグル（galaxyの太陽相当）。再生中は強くパルス。**液滴モデルの分裂演出**: kick/snareヒットで球→楕円に伸長→中央がくびれて戻る。criticalに比例した確率で本当に分裂（2つのローブ群が飛び離れ＋黄の中性子スパーク＋フラッシュ→約0.5秒で再集合）。当たり判定要素は固定し内側の描画グループのみアニメ。
  ...
  - CRITICAL = 左上の**引き裂かれる原子ゲージ**（v0.1.1で制御棒から刷新）: 融合した小さな核クラスタを上下ドラッグで2つの破片半球に引き裂く。分離量∝critical、間に黄色のネッキング光糸＋中性子スパーク。%表示、>0で警告イエロー化。高criticalで原子核が黄白に燃え、ガイガー的な微小フラッシュ＋中性子光条が増える。FXパネルの #critical-slider と双方向ミラー。
  - **壊れていく演出（ブレイクビーツ表現）**: snare/ghostヒットでシーン全体が微小ジョルト（変位∝critical、リトリガーは疑似ランダムのエコーパルスで表現）。critical>0.5で: グリッチスライス（#scene-rootの<use>クローン2枚を横帯クリップ＋±2..7pxオフセットで130msフラッシュ）、電子軌道リングのひび割れdash＋明滅、点灯ノードの破片化フリッカー。critical=0では既定のエレガントな見た目をほぼ維持。
